import { s as supabaseAdmin } from '../../chunks/supabase_BcKHrXOX.mjs';
...
async function GET({
  ...
  try {
    ...
    const {
      data,
      error
    } = await supabaseAdmin.from("site_config").select("data").eq("key", key).single();
...
async function POST({
  ...
  try {
    ...
    const data = body.key ? body.value : body;
    const {
      data: existing,
      error: selectError
    } = await supabaseAdmin.from("site_config").select("id").eq("key", key).single();
    ...
    let error;
    if (existing) {
      const res = await supabaseAdmin.from("site_config").update({
        data
      }).eq("key", key);
      error = res.error;
    } else {
      const res = await supabaseAdmin.from("site_config").insert({
        key,
        data
      });
      error = res.error;
